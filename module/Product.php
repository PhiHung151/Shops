...
class Product
{
    ...
    public function add_new_product_todatabase($phanloai, $ten, $img, $giaban, $tinhtrang, $soluong, $posts, $thuonghieu, $trongluong, $phukien) {
        if (!$this->getconect) {
            ...
            $sql = "INSERT INTO sanpham (id_phanloai, ten, img, giaban, tinhtrang, soluong, posts, thuonghieu, trongluong, phukien)
                    VALUES ($phanloai, '$ten', '$img', $giaban, '$tinhtrang', $soluong, '$posts', '$thuonghieu', '$trongluong', '$phukien')";
    
            mysqli_query($this->getconect, $sql);
    
            if (mysqli_errno($this->getconect) == 0) {
                return true;
            } else {
                echo "Lỗi thực hiện truy vấn: " . mysqli_error($this->getconect);
            }
        }
